...
def reset_dashboard():
    try:
        requests.delete(f"{USER_SERVICE_URL}/reset")
        requests.delete(f"{PRODUCT_SERVICE_URL}/reset")
        requests.delete(f"{ORDER_SERVICE_URL}/reset")
...
def dashboard():
    ...
    # 🧠 Fix: Convert invalid product types to dicts with safe values
    for order in orders:
        if isinstance(order.get("product"), str):
            order["product"] = {"name": order["product"], "price": 0}
...
def add_order():
    ...
    requests.post(f"{ORDER_SERVICE_URL}/add_order", json={"user_id": user_id, "product_id": product_id})
    return redirect(url_for('dashboard'))
